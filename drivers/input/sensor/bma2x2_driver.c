...
static void bma2x2_irq_work_func(struct work_struct *work)
{
	...
	case 0x10:
		...
		if (kobject_uevent_env(&bma2x2->input->dev.kobj, KOBJ_CHANGE, gsensor_wakeup_gesture)) {
			...
			printk(KERN_INFO "kobj : %s\n",bma2x2->input->dev.kobj.name);
			printk(KERN_INFO "parent_kobj : %s\n",bma2x2->input->dev.kobj.parent->name);
			printk(KERN_INFO "parent_parent_kobj : %s\n",bma2x2->input->dev.kobj.parent->parent->name);
		}
#endif
#endif
		break;
		...
		const struct i2c_device_id *id)
{
	...
	bma2x2_set_Int_Enable(client, 11, 1); //flat int enable
