		...
		const struct i2c_device_id *id)
{
	...
	ret = gpio_direction_output(platform_data->ven_gpio,1);
	ret = gpio_direction_output(platform_data->firm_gpio,0);
	//[START] garam.kim@lge.com For pull down IRQ pin
	...
	//[END] garam.kim@lge.com For pull down IRQ pin
	ret = gpio_direction_input(platform_data->irq_gpio);
 ...
 * byungchul.park@lge.com 20120328
 */
//[START] garam.kim@lge.com To know boot mode
	pr_info("%s : lge_get_fboot_mode : %d ,lge_get_nv_qem : %d\n", __func__,lge_get_fboot_mode(), lge_get_nv_qem());
	if((lge_get_fboot_mode() == first_boot) || (lge_get_nv_qem()==1)) {
#ifdef CONFIG_LGE_NFC_MULTICORE_FASTBOOT
	{
		struct task_struct *th;
		th = kthread_create(pn544_factory_standby_set_thread, NULL, "pn544_factory_standby");
...
//[END] garam.kim@lge.com To know boot mode
/* LGE_CHANGE_E */
	return 0;
